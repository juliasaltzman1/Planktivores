...
# READ IN DATA ------------------------------------------------------------
cocos_environmental <- read.csv("~/PLANKTIVORES_COCOS/cocos_environmental.csv")
...
mobula_aic <- aictab(cand.set, modnames=list, second.ord=TRUE, nobs=NULL, sort=TRUE)
print(mobula_aic)
gt(mobula_aic)
...
mantas_aic <-aictab(cand.set, modnames=list, second.ord=TRUE, nobs=NULL, sort=TRUE)
...
whalesharks_aic <- aictab(cand.set, modnames=list, second.ord=TRUE, nobs=NULL, sort=TRUE)
...
exp( 6.204e-02 )
tab_model(GLOBAL_WHALESHARKS_ZINB_NO_LUNAR)
...
sum(cocos_environmental$WhaleSharks, na.rm = TRUE)
